...
public class KanbanPnBase : BaseEntity
{
    public async Task Create(KanbanPnDbContext dbContext)
    {
    ...
    public async Task Update(KanbanPnDbContext dbContext)
    {
        await UpdateInternal(dbContext);
    }

    public async Task Delete(KanbanPnDbContext dbContext)
    {
        await UpdateInternal(dbContext, Constants.WorkflowStates.Removed);
    }

    private async Task UpdateInternal(KanbanPnDbContext dbContext, string state = null)
    {
        if (state != null)
        {
            WorkflowState = state;
    ...
    private object MapVersion(object obj)
    {
        ...
        {
            if (!prop.PropertyType.FullName.Contains("Microting.KanbanBase.Infrastructure.Data.Entities"))
            {
                try
                {
